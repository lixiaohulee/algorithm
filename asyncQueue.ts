...
class AsyncQueue {
  ...
  tap(name: string, fn: (cb: Function) => unknown): AsyncQueue {
    if (this.eventMap.has(name)) {
      const arr = this.eventMap.get(name) as Function[];
      arr.push(fn);
  ...
  exec(name: string, cb: Function) {
    this.run(name, cb);
  }

  private run(name: string, cb: Function) {
    const taskArr = this.eventMap.get(name);
    if (taskArr && taskArr.length) {
      const task = taskArr.shift() as Function;
